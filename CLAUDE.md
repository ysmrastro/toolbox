...
ニュートン式反射望遠鏡の光軸を、ネジ単位で動かして確かめる。詳細は `collimation/README.md`。
触る前に読むべき点だけ挙げる。

**数値の前提は第1回・第2回（公開済み）だけに置く。** 交点から焦点面まで 190mm、
オフセット 2.714mm。**書きかけの記事から出た値を既定値にしない。**
一度、書きかけの記事のために写真から逆算した値を既定値にし、そこから出る結論まで
公開ページに書いてしまって差し戻している。**ここは公開物で、記事より先に出てしまう。**

**土台の柱の長さと太さは、絵のためだけの仮の値。**本体の太さだけは仮定がいらない ──
45°に切ると鏡の楕円になる大きさ（＝斜鏡の短半径）に取ってある。

**寸法以外はすべて仮の値。** `L`・`EYE`・`EPB_R`・`EPB_H` は未実測。画面にもそう書いてある。
`OFFSET` は `L` から決まる従属値なので、`L` を変えたら必ず一緒に直す。

**検算は手で解ける値と突き合わせる**（切り口が鏡の中心に来るオフセット 2.714mm、
切り口／斜鏡 0.8530 / 0.8487、鏡の余り 4.68 / 3.40mm）。**幾何を触ったら再確認する。**

**オフセットはスパイダーで作らない。** 4本は均一に張ってあり、台は筒の中心軸の上にある。
オフセットは**鏡を土台のどこに貼るか**で作る。台ごと寄せると、スパイダーの交点（2回反射）が
主鏡の中心から 4% 以上外れる。構造どおりなら 0%。この形で一度間違えている。

**斜鏡の回転は「土台の柱ごと回す」動き。** 中央の引きネジをゆるめて柱を回すと、45°に切った面が
まるごと回って鏡の法線が円錐を描く。楕円の向きだけが変わるのではなく**視野そのものが動く**
（1°で主鏡の像が視野の 12.8%）。最初は面の中で鏡だけ回す形にしていて、これも直している。

**3回反射の像は目より先に結ぶ実像になる。** 投影で光線の向きを裏返さないと「目の後ろ」と
判定されて、3回反射の層がまるごと描かれない。

**読み取りは重心ではなく円のあてはめ。** 射影された楕円は点の並びが偏るので、重心を中心にすると
斜鏡の縁の偏心が化ける。
