...
ニュートン式反射望遠鏡の光軸を、ネジ単位で動かして確かめる。note 連載「ニュートン式反射望遠鏡」
第3回で詰めた**反射の回数で層に分けるモデル**をそのまま実装してある。詳細は
`collimation/README.md`。触る前に読むべき点だけ挙げる。

**合っている状態の見え方は実機の実写と照合してある**（斜鏡の縁の偏心 1.45%／実測 1.31%、
シルエットの偏心 4.36%／4.73%、輪の太さ 1.21倍・主鏡側／1.29倍・主鏡側 など）。
**幾何に手を入れたら、この表を必ず再確認する。**

**オフセットはスパイダーで作らない。** 4本は均一に張ってあり、台は筒の中心軸の上にある。
オフセットは**鏡を土台のどこに貼るか**で作る。台ごと寄せて作ると、スパイダーの交点（2回反射）が
主鏡の中心から 4.4% 外れる（実測は 1.68%）。この形で一度間違えている。
...
**合っている状態でも、光の切り口は斜鏡のまんなかに来ない。** バグではない。切り口が中心に来る
オフセットは 2.54mm で、仮定しているのは 3.95mm（写真からの逆算）。その差 1.41mm ぶん筒先側へ
寄る。分かりにくいので斜鏡ペインの下に数字と理由を出してある。

**3回反射の像は目より先に結ぶ実像になる。** 投影で光線の向きを裏返さないと「目の後ろ」と
判定されて、3回反射の層がまるごと描かれない。

**読み取りは重心ではなく円のあてはめ。** 射影された楕円は点の並びが偏るので、重心を中心にすると
斜鏡の縁の偏心が 1.45% → 7.90% に化ける。
